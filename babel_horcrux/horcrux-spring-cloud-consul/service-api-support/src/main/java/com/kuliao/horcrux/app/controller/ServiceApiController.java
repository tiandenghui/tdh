...
public class ServiceApiController {
     ...
     * 获取所有服务,剔除所有无效的服务实例
     */
    @RequestMapping(value = "/allservicerm", method = RequestMethod.GET)
    public void getAllServicer() {
        ...
        for (int i = 0; i < members.size(); i++) {
            ...
            LOGGER.info("{}机器的role为：{}=====注释*role为consul代表服务端   role为node代表客户端", address, role);
            ...
            if (role.equals("node")) {
                ...
                while (it.hasNext()) {
                    ...
                    LOGGER.debug("在{}客户端上的服务名称 :{}**服务ID:{}", address, serviceName, serviceId);
                    //根据服务名称获取服务的健康检查信息
                    ...
                    LOGGER.info("在{}客户端上的服务 :{}的健康状态值：{}", address, serviceName, checkStatus);
                    if (checkStatus != Check.CheckStatus.PASSING) {
                        LOGGER.info("在{}客户端上的服务 :{}为无效服务，准备清理...................", address, serviceName);
